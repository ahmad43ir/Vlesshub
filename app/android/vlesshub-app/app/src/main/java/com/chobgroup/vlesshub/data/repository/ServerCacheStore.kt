...
import com.chobgroup.vlesshub.data.model.ClientCompatibility
import com.chobgroup.vlesshub.data.model.ConfigFormat
import com.chobgroup.vlesshub.data.model.TransportType
import com.chobgroup.vlesshub.data.model.ValidationStatus
...
class ServerCacheStore private constructor() {
    ...
    fun linksSortByPing(): Boolean = prefs.getBoolean(KEY_LINKS_SORT_BY_PING, false)

    fun setLinksSortByPing(value: Boolean) {
        prefs.edit().putBoolean(KEY_LINKS_SORT_BY_PING, value).apply()
    }

    fun resetLinksSortByPing() {
        prefs.edit().remove(KEY_LINKS_SORT_BY_PING).apply()
    }

    // ── Links pagination (persisted — survives leaving/reopening the app;
    //    explicitly reset by the Links refresh action) ────────────────

    fun linksVisibleCount(): Int = prefs.getInt(KEY_LINKS_VISIBLE_COUNT, DEFAULT_LINKS_VISIBLE_COUNT)

    fun setLinksVisibleCount(value: Int) {
        prefs.edit().putInt(KEY_LINKS_VISIBLE_COUNT, value.coerceAtLeast(DEFAULT_LINKS_VISIBLE_COUNT)).apply()
    }

    fun resetLinksVisibleCount() {
        prefs.edit().remove(KEY_LINKS_VISIBLE_COUNT).apply()
    }
    ...
    companion object {
        ...
        private const val KEY_LINKS_VISIBLE_COUNT = "links_visible_count"
        private const val KEY_LINKS_SORT_BY_PING = "links_sort_by_ping"
        private const val DEFAULT_LINKS_VISIBLE_COUNT = 10
...
private fun VpnServer.toJson(): JSONObject = JSONObject().apply {
    ...
    put("validationStatus", validationStatus.name.lowercase())
    if (realDelayMs != null) put("realDelayMs", realDelayMs)
    if (validationError != null) put("validationError", validationError)
    if (validatedAt != null) put("validatedAt", validatedAt)
    if (xrayVersion != null) put("xrayVersion", xrayVersion)
...
private fun JSONObject.toVpnServer(): VpnServer = VpnServer(
    ...
    transport = TransportType.fromString(optString("transport")),
    pingMs = if (has("pingMs")) optInt("pingMs") else null,
    validationStatus = ValidationStatus.fromString(optString("validationStatus")),
    realDelayMs = if (has("realDelayMs")) optInt("realDelayMs") else null,
    validationError = if (has("validationError")) optString("validationError").takeIf { it.isNotBlank() } else null,
    validatedAt = if (has("validatedAt")) optString("validatedAt").takeIf { it.isNotBlank() } else null,
    xrayVersion = if (has("xrayVersion")) optString("xrayVersion").takeIf { it.isNotBlank() } else null,
    clientCompatibility = ClientCompatibility.forTransport(TransportType.fromString(optString("transport"))),
