...
object ConfigNormalizer {
    ...
    /**
     * Detect the transport type from a raw config URI.
     * Returns the transport as a [TransportType] enum value.
     */
    fun detectTransport(raw: String): TransportType {
        val trimmed = raw.trim()
        val qIdx = trimmed.indexOf('?')
        if (qIdx < 0) return TransportType.TCP
        val query = trimmed.substring(qIdx + 1)
        val params = parseQuery(query)
        val transport = params["type"]?.lowercase() ?: return TransportType.TCP
        return TransportType.fromString(transport)
    }
