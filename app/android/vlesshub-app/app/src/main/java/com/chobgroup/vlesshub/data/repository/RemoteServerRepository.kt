...
class RemoteServerRepository : ServerRepository {
    ...
    private fun mapServers(array: JSONArray): List<VpnServer> {
        ...
        for (i in 0 until array.length()) {
            ...
            val transport = runCatching { ConfigNormalizer.detectTransport(config) }.getOrDefault(TransportType.UNKNOWN)
            result += VpnServer(
                ...
                transport = transport,
                validationStatus = ValidationStatus.NEEDS_REAL_TEST,
                clientCompatibility = ClientCompatibility.forTransport(transport),
