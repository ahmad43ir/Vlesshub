 ...
 * routed to the VlessHub Telegram bot — no email involved.
 */
@Composable
fun SettingsScreen() {
    ...
    /** Channel-owner requests are handled in the VlessHub Telegram bot. */
    fun openVlesshubBot() {
        openUrl(AppConstants.VLESSHUB_BOT_URL)
    }
    ...
    Column(
        ...
        SettingsRow(
            ...
            subtitle = "Message @Vless_hub_bot — we'll purge your configs",
            icon = AppIcons.OpenInNew,
        ) { openVlesshubBot() }
        SectionDivider()
        SettingsRow(
            title = "Add my Telegram channel",
            subtitle = "Message @Vless_hub_bot — share fresh configs with VlessHub users",
            icon = AppIcons.OpenInNew,
        ) { openVlesshubBot() }
        Spacer(Modifier.height(20.dp))
